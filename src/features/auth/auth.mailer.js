          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border-radius:16px;overflow:hidden;">
            <tr>
              <td style="padding:38px 40px 28px;text-align:center;">
                ...
                <div style="height:1px;background:#e8ecec;margin:28px 0 54px;"></div>
                <h1 style="margin:0;font-size:22px;line-height:28px;font-weight:700;color:${brand.textPrimary};">비밀번호 재설정 인증 코드</h1>
                <p style="margin:18px auto 44px;max-width:360px;font-size:14px;line-height:22px;font-weight:500;color:${brand.textSecondary};word-break:keep-all;">비밀번호 재설정을 계속하려면 아래 인증 코드를 입력해주세요.</p>
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:320px;margin:0 auto;table-layout:fixed;">
                  <tr>${codeCells}</tr>
                </table>
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border-radius:16px;overflow:hidden;">
            <tr>
              <td style="padding:38px 40px;text-align:center;">
                <div style="font-size:22px;line-height:28px;font-weight:700;color:${brand.main};">DonWorry</div>
                <div style="height:1px;background:#e8ecec;margin:28px 0 54px;"></div>
                <h1 style="margin:0;font-size:22px;line-height:28px;font-weight:700;color:${brand.textPrimary};">카카오 로그인 안내</h1>
                <p style="margin:18px auto 0;max-width:380px;font-size:14px;line-height:22px;font-weight:500;color:${brand.textSecondary};word-break:keep-all;">요청하신 이메일은 카카오 로그인으로 가입된 계정입니다.<br>비밀번호 재설정 없이 카카오 로그인을 이용해 주세요.</p>
              </td>
            </tr>
          </table>
        </td>
...
const sendMail = async ({ email, subject, text, html, productionErrorMessage }) => {
...
export const sendEmailVerificationCode = async ({ email, code, codeTtlSeconds }) => {
  return sendMail({
    ...
    productionErrorMessage: 'SMTP configuration is required to send email verification code.',
  });
};
...
export const sendPasswordResetCode = async ({ email, code, codeTtlSeconds }) => {
  return sendMail({
    email,
    subject: 'DonWorry 비밀번호 재설정 인증 코드',
    text: [
      'DonWorry',
      '비밀번호 재설정 인증 코드',
      '비밀번호 재설정을 계속하려면 아래 인증 코드를 입력해주세요.',
      code,
      `이 코드는 ${formatTtlDuration(codeTtlSeconds)} 뒤에 만료됩니다.`,
      '직접 요청하신 경우가 아니라면 이 메일을 무시하셔도 됩니다.',
    ].join('\n'),
...
export const sendKakaoLoginGuide = async ({ email }) => {
  return sendMail({
    email,
    subject: 'DonWorry 카카오 로그인 안내',
    text: [
      'DonWorry 카카오 로그인 안내',
      '요청하신 이메일은 카카오 로그인으로 가입된 계정입니다.',
      '비밀번호 재설정 없이 카카오 로그인을 이용해 주세요.',
    ].join('\n'),
    html: createKakaoLoginGuideHtml(),
    productionErrorMessage: 'SMTP configuration is required to send Kakao login guide.',
  });
};
